...
@Component({
  selector: 'app-boat-owner-calendar-page',
  templateUrl: './boat-owner-calendar-page.component.html',
  styleUrls: ['./boat-owner-calendar-page.component.scss']
})
export class BoatOwnerCalendarPageComponent implements OnInit {
  ...
  private getEvents() {
    ...
    this.boatService.getAllPromotionsForBoatOwner().subscribe((data:any) => {
      data.forEach((element: any) => {
        let id = element.id || 0;
        this.addPromotionToCalendar(id, element.fromTime, element.toTime, element.boat.name, element.price, element.validUntil);
      });
    })
